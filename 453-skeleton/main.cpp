...
glm::vec3 raytraceSingleRay(Scene const &scene, Ray const &ray, int level, int source_id) {
    ...
    glm::vec3 shading_colour, reflected_colour, transmitted_colour, q, n;

    if(level < 1) return glm::vec3(0, 0, 0);

    // Shadow ray from intersection point to light source
    glm::vec3 origin_shadow = result.point;
    glm::vec3 direction_shadow = glm::normalize(scene.lightPosition - result.point);
    Ray shadow_ray(origin_shadow, direction_shadow);
    Intersection result_2 = getClosestIntersection(scene, shadow_ray, result.id);

    if(result.numberOfIntersections == 0) return glm::vec3(0, 0, 0); // black;

    if (scene.current_scene == 1)
    {
        // If shadow ray is blocked by object (has intersections) then exclude diffuse and specular lighting
        if (result_2.numberOfIntersections > 0)
        {
            // Ensure the intersection is within the frame and not behind the light source by comparing distances of alleged intersection and maximum possible distance of intersection
            float intersection_distance = sqrt(glm::length(result_2.point - origin_shadow));

            float max_distance = sqrt(glm::length(scene.lightPosition - origin_shadow));
    ...
    else
    {
        ...
        if (result_2.numberOfIntersections > 0)
        {
            ...
            if (intersection_distance < max_distance)
            {
                shading_colour = phong.Ia();
            }
        }
        else
        {
            shading_colour = phong.I();
        }

        q = result.point;
        n = result.normal;
        // r = l - 2(n.l)n
        Ray r = Ray(ray.origin, glm::normalize(ray.direction - (2 * glm::dot(n, ray.direction) * n)));
        // t = // transmit ray

        reflected_colour = raytraceSingleRay(scene, r, level - 1, result.id) * result.material.reflectionStrength;
        // transmitted_colour = raytraceSingleRay(scene, t, level-1, result.id);

        // return (shading_colour + reflected_colour + transmitted_colour);
        return (shading_colour + reflected_colour);
    }
}

struct RayAndPixel {
	Ray ray; // Ray is a struct of origin, direction
	int x;
	int y;
};

glm::vec3 normaliseThis(glm::vec3 vec)
{
    float magnitude = sqrt(pow(vec.x,2) + pow(vec.y,2) + pow(vec.z,2)); // z value is always 1
...
std::vector<RayAndPixel> getRaysForViewpoint(Scene const &scene, ImageBuffer &image, glm::vec3 viewPoint) {
    ...
    for (float i = -0.5; x < width; x++)
    {
        y = 0;
        for (float j = -0.5; y < height; y++)
        {
