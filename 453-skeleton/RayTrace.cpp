...
Intersection Sphere::getIntersection(Ray ray)
{
    ...
    // discriminant = (d · D)^2 − ‖D‖^2 + r^2
    glm::vec3 D = ray.origin - centre;
    glm::vec3 d = ray.direction;
    float r = radius;
    float n_D = glm::length(D);
    float discriminant = pow(glm::dot(d,D),2) + pow(r,2) - pow(glm::length(D),2);

    /*

        If the discriminant is negative, then the ray does not intersect the sphere. Otherwise, the sphere is intersected at p0 + td for each t value; the smallest t value gives the closest intersection.
    ...
    */
    ...
    if (discriminant < 0)
    {
        ...
        i.point = glm::vec3(0.f, 0.f, 0.f);
        i.normal = glm::vec3(0.f, 0.f, 0.f);
    }
    else 
    {
        i.numberOfIntersections = 1;

        float t1 =  -(glm::dot(d, D)) + sqrt(discriminant);
        float t2 =  -(glm::dot(d, D)) - sqrt(discriminant);

        if (t1 < t2 && t1 >= 0)
        {
            i.point = ray.origin + t1 * d;
            i.normal = glm::normalize(i.point - centre);
        }
        else if (t2 >= 0)
        {
            i.point = ray.origin + t2 * d;
            i.normal = glm::normalize(i.point - centre);
        }
        else
        {
